...
class MemberControllerTest extends TestConfig {
	...
	@Test
	@DisplayName("2. 회원가입 중복 회원 실패 테스트")
	...
	@Test
	@DisplayName("3. 회원가입 입력값 유효성 실패 테스트")
	public void validMemberRegisterTest() throws Exception {
		...
		given(spec).log().all()
			.filter(RestAssuredRestDocumentationWrapper.document("회원 가입 API - 실패: 입력 유효성 체크",
				RestAssuredRestDocumentationWrapper.resourceDetails()
					.tag("회원 API")
					.summary("회원 가입"),
				requestFields(
